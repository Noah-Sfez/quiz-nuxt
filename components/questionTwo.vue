...
onMounted(async () => {
  ...
  if (quiz.value && quiz.value.quiz && quiz.value.quiz.questions && quiz.value.quiz.questions.length > 0) {
    secondQuestion.value = quiz.value.quiz.questions[1]
  }
})
</script>
...
<template>
  <div>
    <h1>Deuxième Question</h1>
    <div v-if="secondQuestion">
      <h2>{{ secondQuestion.question }}</h2>
      <ul>
        <li v-for="(answer, index) in secondQuestion.answers" :key="index">{{ answer }}</li>
      </ul>
